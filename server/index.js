...
app.get("/entries/user/:user_id", async (req, res) => {
  try {
    ...
    const allEntries = await pool.query(
      "SELECT * FROM shoppinglist WHERE user_id = $1 ORDER BY priority_flag DESC, created_at ASC",
      [user_id]
    );
...
app.get("/entries/:id", async (req, res) => {
  try {
    ...
    const entry = await pool.query(
      "SELECT * FROM shoppinglist WHERE entry_id = $1",
      [id]
    );
...
app.patch("/entries/:id", async (req, res) => {
  try {
    ...
    const query = `UPDATE shoppinglist SET ${updateFields.join(
      ", "
    )}, updated_at = CURRENT_TIMESTAMP WHERE entry_id = $${values.length}`;
...
app.delete("/entries/:id", async (req, res) => {
  try {
    ...
    const deleteEntry = await pool.query(
      "DELETE FROM shoppinglist WHERE entry_id = $1",
      [id]
    );
